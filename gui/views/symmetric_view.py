...
class SymmetricView(QWidget):
    ...
    def setup_encrypt_tab(self):
        ...
        self.key_hex_check.toggled.connect(
            lambda checked: self.key_b64_check.setChecked(False) if checked else None
        )
        self.key_b64_check.toggled.connect(
            lambda checked: self.key_hex_check.setChecked(False) if checked else None
        )
        ...
        self.iv_hex_check.toggled.connect(
            lambda checked: self.iv_b64_check.setChecked(False) if checked else None
        )
        self.iv_b64_check.toggled.connect(
            lambda checked: self.iv_hex_check.setChecked(False) if checked else None
        )
    ...
    def setup_decrypt_tab(self):
        ...
        self.decrypt_mode_combo.addItems([mode.value for mode in Mode])
        ...
        self.decrypt_key_hex_check.toggled.connect(
            lambda checked: (
                self.decrypt_key_b64_check.setChecked(False) if checked else None
            )
        )
        self.decrypt_key_b64_check.toggled.connect(
            lambda checked: (
                self.decrypt_key_hex_check.setChecked(False) if checked else None
            )
        )
        ...
        self.decrypt_iv_hex_check.toggled.connect(
            lambda checked: (
                self.decrypt_iv_b64_check.setChecked(False) if checked else None
            )
        )
        self.decrypt_iv_b64_check.toggled.connect(
            lambda checked: (
                self.decrypt_iv_hex_check.setChecked(False) if checked else None
            )
        )
    ...
    def update_algorithm_options(self):
        ...
        self.key_size_combo.setEnabled(key_size_visible)
        ...
        # 更新AAD输入可见性
        is_gcm_mode = self.mode_combo.currentText() == "GCM"
        self.aad_input.setEnabled(is_gcm_mode)

        # 更新IV输入可见性
        needs_iv = self.mode_combo.currentText() in ["CBC", "CFB", "OFB", "CTR", "GCM"]
    ...
    def update_decrypt_algorithm_options(self):
        ...
        # 更新密钥长度选项可见性
        key_size_visible = algorithm == "AES"
        self.decrypt_key_size_combo.setEnabled(key_size_visible)
        ...
        # 更新AAD输入可见性
        is_gcm_mode = self.decrypt_mode_combo.currentText() == "GCM"
        self.decrypt_aad_input.setEnabled(is_gcm_mode)

        # 更新IV输入可见性
        needs_iv = self.decrypt_mode_combo.currentText() in [
            "CBC",
            "CFB",
            "OFB",
            "CTR",
    ...
    def generate_random_key(self):
        ...
        try:
            ...
            if self.key_hex_check.isChecked():
                ...
                # 尝试显示为UTF-8，如果不可显示则使用十六进制
                try:
                    self.key_input.setText(random_key.decode("utf-8"))
    ...
    def generate_random_iv(self):
        ...
        try:
            ...
            if self.iv_hex_check.isChecked():
                ...
                # 尝试显示为UTF-8，如果不可显示则使用十六进制
                try:
                    self.iv_input.setText(random_iv.decode("utf-8"))
                except UnicodeDecodeError:
                    self.iv_input.setText(random_iv.hex())
    ...
    def get_decrypt_key_bytes(self):
        """获取解密密钥字节数据"""
        key_text = self.decrypt_key_input.text()
        if not key_text:
            raise ValueError("密钥不能为空")
    ...
    def get_iv_bytes(self):
        ...
        if self.iv_hex_check.isChecked():
            ...
            return iv_text.encode("utf-8")

    def get_decrypt_iv_bytes(self):
        """获取解密IV字节数据"""
        iv_text = self.decrypt_iv_input.text()
        if not iv_text:
            return None
    ...
    def get_aad_bytes(self):
        """获取AAD字节数据"""
        aad_text = self.aad_input.text()
        if not aad_text:
            return None
        return aad_text.encode("utf-8")

    def get_decrypt_aad_bytes(self):
        """获取解密AAD字节数据"""
        aad_text = self.decrypt_aad_input.text()
        if not aad_text:
            return None
        return aad_text.encode("utf-8")
        ...
        try:
            ...
            if self.text_radio.isChecked():
                ...
                if self.base64_output_check.isChecked():
                    result_text = base64.b64encode(encrypted).decode("ascii")
                elif self.hex_output_check.isChecked():
                    result_text = encrypted.hex()
                else:
                    # 默认使用Base64
                    result_text = base64.b64encode(encrypted).decode("ascii")
                    self.base64_output_check.setChecked(True)
                ...
                if not input_file or not os.path.exists(input_file):
                    self.result.setText("请选择有效的输入文件")
                    return

                if not output_file:
                    self.result.setText("请指定输出文件路径")
                    return
                ...
                self.result.setText(f"文件已成功加密并保存到:\n{output_file}")

        except Exception as e:
            self.result.setText(f"加密错误: {str(e)}")
    ...
    def perform_decryption(self):
        ...
        try:
            ...
            if self.decrypt_text_radio.isChecked():
                # 文本模式解密
                text = self.decrypt_text_input.toPlainText()
                if not text:
                    self.decrypt_result.setText("请输入要解密的密文")
                    return
                ...
                # 保存结果
                self.last_decrypt_result = decrypted

                # 显示结果
                try:
                    # 使用选定的编码
                    encoding = self.decrypt_encoding_combo.currentText()
                    result_text = decrypted.decode(encoding)
                ...
                output_file = self.decrypt_output_file_path.text()

                if not input_file or not os.path.exists(input_file):
                    self.decrypt_result.setText("请选择有效的输入文件")
                    return

                if not output_file:
                    self.decrypt_result.setText("请指定输出文件路径")
                    return
                ...
                self.decrypt_result.setText(f"文件已成功解密并保存到:\n{output_file}")

        except Exception as e:
            self.decrypt_result.setText(f"解密错误: {str(e)}")
    ...
    def copy_decrypt_result(self):
        """复制解密结果到剪贴板"""
        text = self.decrypt_result.toPlainText()
        if text:
            clipboard = QApplication.clipboard()
            clipboard.setText(text)
