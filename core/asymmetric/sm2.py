...
def _hex_to_bn(hex_str: str) -> c_void_p:
    """将十六进制字符串转换为BIGNUM"""
    bn_ptr = c_void_p(0)
    hex_cstr = c_char_p(hex_str.encode("ascii"))
    result = libcrypto.BN_hex2bn(ctypes.byref(bn_ptr), hex_cstr)
    if result == 0 or not bn_ptr.value:
        raise ValueError("无法将十六进制字符串转换为BIGNUM")
    return bn_ptr.value


def _bytes_to_bn(data: bytes) -> c_void_p:
    """将字节串转换为BIGNUM"""
    bn = libcrypto.BN_new()
    if not bn:
        raise MemoryError("无法分配BIGNUM内存")

    libcrypto.BN_bin2bn(data, len(data), bn)
    return bn
...
class SM2(AsymmetricCipher):
    ...
    def load_public_key(
        ...
        Args:
            key_data: 密钥数据
            format: 格式 ('pem', 'der', 'xml', 'hex', 'hex-dict')
        ...
        try:
            if isinstance(key_data, str):
                if format.lower() == "xml":
                    ...
                    if curve_name != SM2_CURVE_NAME:
                        raise ValueError(f"不支持的曲线: {curve_name}")

                    # 获取坐标点
                    x_bytes = base64.b64decode(root.find("X").text)
                    y_bytes = base64.b64decode(root.find("Y").text)
                    ...
                    if not point:
                        libcrypto.EC_KEY_free(key)
                        raise ValueError("创建EC_POINT失败")

                    # 转换坐标为BIGNUM
                    x_bn = _bytes_to_bn(x_bytes)
                    y_bn = _bytes_to_bn(y_bytes)

                    # 设置点的坐标
                    result = libcrypto.EC_POINT_set_affine_coordinates(
                        group, point, x_bn, y_bn, None
                    )

                    # 释放BIGNUM
                    libcrypto.BN_free(x_bn)
                    libcrypto.BN_free(y_bn)

                    if result != 1:
                        libcrypto.EC_POINT_free(point)
                        libcrypto.EC_KEY_free(key)
                        raise ValueError("设置点坐标失败")

                    # 设置公钥
                    result = libcrypto.EC_KEY_set_public_key(key, point)
                    libcrypto.EC_POINT_free(point)
                    ...
                    return SM2Key(key, "public")

                elif format.lower() == "hex":
                    # 解析纯十六进制格式
                    # 去除可能的空白字符
                    hex_str = key_data.strip()
                    ...
                    if not group:
                        libcrypto.EC_KEY_free(key)
                        raise ValueError("获取EC组失败")

                    # 从十六进制字符串创建点
                    hex_cstr = c_char_p(hex_str.encode("ascii"))
                    point = libcrypto.EC_POINT_hex2point(group, hex_cstr, None, None)
                    if not point:
                        libcrypto.EC_KEY_free(key)
                        raise ValueError("无法从十六进制字符串创建EC点")
            ...
            # 创建BIO
            bio = libcrypto.BIO_new_mem_buf(key_data, len(key_data))
            if not bio:
                raise ValueError("创建BIO失败")

            try:
                key = None
                if format.lower() == "pem":
                    # 从PEM读取公钥
                    key = libcrypto.PEM_read_bio_EC_PUBKEY(bio, None, None, None)
                elif format.lower() == "der":
                    # 从DER读取公钥
                    p_key = ctypes.pointer(c_void_p())
                    p_data = ctypes.cast(key_data, POINTER(c_char_p))
                    key = libcrypto.d2i_EC_PUBKEY(p_key, p_data, len(key_data))
                elif format.lower() == "openssh":
                    logger.warning("SM2不支持OpenSSH格式, 尝试作为PEM格式加载")
                    key = libcrypto.PEM_read_bio_EC_PUBKEY(bio, None, None, None)
                else:
                    raise ValueError(f"不支持的格式: {format}")

                if not key:
                    raise ValueError(f"加载SM2公钥失败")

                # 验证曲线类型
                group = libcrypto.EC_KEY_get0_group(key)
                if hasattr(libcrypto, "EC_GROUP_get_curve_name"):
                    curve_nid = libcrypto.EC_GROUP_get_curve_name(group)
                    if curve_nid != SM2_CURVE_NID:
                        libcrypto.EC_KEY_free(key)
                        raise ValueError(f"提供的公钥不是SM2曲线")
            ...
            logger.error(f"加载SM2公钥失败: {e}")
            raise ValueError(f"加载SM2公钥失败: {e}")
        ...
        try:
            if isinstance(key_data, str):
                if format.lower() == "xml":
                    ...
                    x_bytes = base64.b64decode(root.find("X").text)
                    y_bytes = base64.b64decode(root.find("Y").text)
                    d_bytes = base64.b64decode(root.find("D").text)
                    ...
                    # 设置私钥
                    result = libcrypto.EC_KEY_set_private_key(key, d_bn)
                    ...
                    if result != 1:
                        libcrypto.EC_KEY_free(key)
                        raise ValueError("设置SM2私钥失败")

                    return SM2Key(key, "private", password)

                elif format.lower() == "hex":
                    # 解析十六进制格式
                    # 十六进制格式只包含私钥D值, 需要重新生成公钥
                    d_hex = key_data.strip()
            ...
            try:
                ...
                if format.lower() == "pem":
                    ...
                    if password:
                        # 如果有密码, 使用密码回调
                        key = libcrypto.PEM_read_bio_ECPrivateKey(
                            bio, None, None, password
                        )
                    else:
                        key = libcrypto.PEM_read_bio_ECPrivateKey(bio, None, None, None)

                    # 如果直接读取失败, 尝试通过EVP_PKEY读取
                    if not key:
                        # 重置BIO位置
                        libcrypto.BIO_free(bio)
                        bio = libcrypto.BIO_new_mem_buf(key_data, len(key_data))
                        ...
                        # 尝试读取PKCS#8格式的私钥
                        evp_pkey = libcrypto.PEM_read_bio_PrivateKey(
                            bio, None, None, password if password else None
                        )

                        if evp_pkey:
                            # 从EVP_PKEY中提取EC_KEY
                            key = libcrypto.EVP_PKEY_get1_EC_KEY(evp_pkey)
                            libcrypto.EVP_PKEY_free(evp_pkey)
                elif format.lower() == "der":
                    # 从DER读取私钥
                    p_key = ctypes.pointer(c_void_p())
                    p_data = ctypes.cast(key_data, POINTER(c_char_p))
                    key = libcrypto.d2i_ECPrivateKey(p_key, p_data, len(key_data))
                elif format.lower() == "openssh":
                    logger.warning("SM2不支持OpenSSH格式, 尝试作为PEM格式加载")
                    key = libcrypto.PEM_read_bio_ECPrivateKey(bio, None, None, None)
                else:
                ...
                if hasattr(libcrypto, "EC_GROUP_get_curve_name"):
                    ...
                    if curve_nid != SM2_CURVE_NID:
                        libcrypto.EC_KEY_free(key)
                        raise ValueError(f"提供的私钥不是SM2曲线")
            ...
            logger.error(f"加载SM2私钥失败: {e}")
            raise ValueError(f"加载SM2私钥失败: {e}")
