...
class HashFactory:
    ...
    def is_algorithm_secure(algorithm: str) -> bool:
        ...
        return HASH_ALGORITHMS[algorithm][1]

    @staticmethod
    def get_hash_length(algorithm: str) -> int:
        """
        获取哈希算法的输出长度（字节）
        ...
        cls, _, _, default_params = HASH_ALGORITHMS[algorithm]
        try:
            # 对于BLAKE2，使用默认的digest_size
            hash_obj = cls(**default_params)
            return len(hash_obj.digest())
        except Exception as e:
            raise RuntimeError(f"无法获取 {algorithm} 的哈希长度: {e}")
